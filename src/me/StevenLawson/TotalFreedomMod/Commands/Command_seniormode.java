...
@CommandParameters(description = "Close server to senioradmins.", usage = "/<command> [on | off]")
public class Command_seniormode extends TFM_Command
{
    ...
    public boolean run(CommandSender sender, Player sender_p, Command cmd, String commandLabel, String[] args, boolean senderIsConsole)
    {
        ...
        else if (args[0].equalsIgnoreCase("on"))
        {
            ...
            TFM_Util.adminAction(sender.getName(), "Closing the server to non-senioradmins.", true);
            for (Player player : server.getOnlinePlayers())
            {
                if (!TFM_AdminList.isSeniorAdmin(player))
                {
                    player.kickPlayer("Server is now closed to senioradmins.");
                }
            }
            return true;
        }
